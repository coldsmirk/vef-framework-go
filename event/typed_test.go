...
func TestSubscribeTypedDecodesPointerFromRawPayload(t *testing.T) {
	...
	require.NoError(t, err, "SubscribeTyped should accept pointer event handlers")

	body, err := json.Marshal(&PointerEvent{Value: "wire"})
	require.NoError(t, err, "Pointer event should marshal to JSON")
	...
	require.NoError(t, bus.dispatch(context.Background(), env), "Raw pointer payload should dispatch successfully")
...
func TestSubscribeTypedDecodesValueFromRawPayload(t *testing.T) {
	...
	require.NoError(t, err, "SubscribeTyped should accept value event handlers")

	body, err := json.Marshal(ValueEvent{Value: "wire-val"})
	require.NoError(t, err, "Value event should marshal to JSON")

	env := event.Envelope{
		Type:    "test.value",
		Payload: event.RawPayload{Type: "test.value", Body: body},
	}
	require.NoError(t, bus.dispatch(context.Background(), env), "Raw value payload should dispatch successfully")
...
func TestSubscribeTypedAcceptsInProcessPointer(t *testing.T) {
	...
	require.NoError(t, err, "SubscribeTyped should accept in-process pointer handlers")

	original := &PointerEvent{Value: "in-process"}
	env := event.Envelope{Type: "test.pointer", Payload: original}
	require.NoError(t, bus.dispatch(context.Background(), env), "In-process pointer payload should dispatch successfully")
...
func TestSubscribeTypedWrapsUnknownPayload(t *testing.T) {
	...
	require.NoError(t, err, "SubscribeTyped should register before mismatched payload dispatch")
	...
	require.Error(t, err, "Mismatched concrete payload should fail dispatch")
...
func TestSubscribeTypedDecodeFailureWrapsError(t *testing.T) {
	...
	require.NoError(t, err, "SubscribeTyped should register before invalid JSON dispatch")
...
func TestAsEventsConvertsTypedSlice(t *testing.T) {
	...
	require.Same(t, src[1], out[1], "AsEvents should preserve each source item identity")
}
