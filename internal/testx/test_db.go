...
// the matching dialect, exposing the raw handle that many crud suites seed
// against directly. It deliberately omits orm.Open's query/SQL-guard hook
// (orm.WithSQLGuard is off by default, so the only divergence is query
// logging): tests that want the production wrapper construct it via orm.New on
// the returned handle. The caller owns closing the returned *sql.DB.
