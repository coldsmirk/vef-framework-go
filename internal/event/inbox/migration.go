...
var expectedTables = []string{"sys_event_inbox"}
...
func Migrate(ctx context.Context, db orm.DB, kind config.DBKind) error {
	...
	if err != nil {
		return fmt.Errorf("event inbox: check migration status: %w", err)
	}
	...
	if _, err := db.NewRaw(sql).Exec(ctx); err != nil {
		return fmt.Errorf("event inbox: execute migration: %w", err)
	}
...
func loadScript(kind config.DBKind) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w %q for sys_event_inbox", errUnsupportedDBKind, kind)
	}
