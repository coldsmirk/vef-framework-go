...
- **Assertions**: suite tests must use instance methods such as `s.Require()` and `s.Equal()`. Non-suite tests use standalone `require` and `assert`. Every assertion needs a descriptive message, and assertion messages must be written in English.
