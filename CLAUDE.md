...
- **Approval node config normalizes at deploy**: `NodeData.ApplyTo` resolves omitted enum fields to designer defaults (the `approval.Default*` constants in `node_data.go`), and the default-true permission toggles (`IsRollbackAllowed`, `IsTransferAllowed`, `IsAddAssigneeAllowed`, `IsRemoveAssigneeAllowed`, `IsManualCCAllowed`) are `*bool` so an omitted field resolves to allowed while explicit `false` survives. Deploy validation (`service/node_config_validation.go`) rejects out-of-enum values and incomplete dependent fields (ratio rule without `passRatio`, `transfer_specified` without `fallbackUserIds`, `transfer_admin` without `adminUserIds`, `specified` rollback without `rollbackTargetKeys`, blank condition subjects/expressions). Field conditions evaluate natively in Go against the `approval.ConditionOperator` contract; expression conditions run through the framework `expression.Engine` (Zen syntax: `formData.field`, `and` / `or` / `not` — not expr-lang `&&`). The flow editor mirrors both the defaults (`normalize-node-data.ts`) and the validation (`flow-validation.ts`); the two sides must stay in lockstep.
